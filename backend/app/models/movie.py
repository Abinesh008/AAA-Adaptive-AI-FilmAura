...
class UserPreferenceProfile(Base):
    """
    User preference weights and content exclusion settings.
    """
    __tablename__ = "user_preference_profiles"
    
    user_id: Mapped[str] = mapped_column(String(100), unique=True, index=True, nullable=False)
    genre_weights: Mapped[dict] = mapped_column(JSON, default=dict, nullable=False)
    favorite_directors: Mapped[list] = mapped_column(JSON, default=list, nullable=False)
    favorite_actors: Mapped[list] = mapped_column(JSON, default=list, nullable=False)
...
class UserInteractionHistory(Base):
    """
    User movie interaction history logging explicit ratings, clicks, skips, and bookmarks.
    """
    __tablename__ = "user_interaction_histories"
    
    user_id: Mapped[str] = mapped_column(String(100), nullable=False, index=True)
    movie_id: Mapped[int] = mapped_column(Integer, nullable=False, index=True)
    interaction_type: Mapped[str] = mapped_column(String(50), nullable=False)  # rating, click, skip, bookmark
    rating: Mapped[float | None] = mapped_column(Float, nullable=True)
